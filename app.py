...
@st.cache_resource
def load_models():
    # Cache resource keeps models in memory after first load
    return {
...
if menu == "🌟 Project Overview":
    ...
    if file and st.button("🚀 Run Diagnosis"):
        # PRESENTATION SAFEGUARD: Clear backend cache before processing
        K.clear_session()
        ...
        with st.spinner("Analyzing Biomarkers..."):
            ...
            # PRESENTATION SAFEGUARD: Free up memory manually
            del arr
            del heatmap
            gc.collect()
